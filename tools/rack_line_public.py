...
def numpy_to_ros_image(img_np, encoding="bgr8"):
    """
    手动将 OpenCV (Numpy) 图片转换为 ROS Image 消息
    替代 cv_bridge，解决 Conda 环境下 libcv_bridge.so 找不到的问题
    """
    msg = Image()
    ...
    msg.is_bigendian = 0
    # 计算 step (一行数据的字节数)
    msg.step = len(img_np[0]) * img_np.itemsize * img_np.shape[2] # width * itemsize * channels
    # 转换数据
...
def run(**kwargs):
    ...
    if ROS_AVAILABLE:
        try:
            ...
            ros_pub = rospy.Publisher('/arm_0/picking/moveview_return', moveview_return, queue_size=1)
            # cv_bridge = CvBridge()
        ...
        for i, det in enumerate(pred):
            ...
            if ros_pub is not None and ROS_AVAILABLE:
                try:
                    ...
                    # 关键修改：使用 numpy_to_ros_image 替代 cv_bridge
